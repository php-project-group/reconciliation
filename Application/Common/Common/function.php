...
function getXml(){
    $dom=new DOMDocument();
    $dom->load('G:\Apache24\htdocs\reconciliation\document.xml');
...
//                print $value->nodeName . " = " . $value->nodeValue."<br />";
            array_push($tem,$value->nodeValue);
            foreach($value->childNodes as $k=>$val){
                ...
                foreach($val->childNodes as $y=>$v){
                    ...
                    if($v->nodeName == 'w:br'){
                        if(count($word)>0){
                            array_unshift($tem,$value->nodeValue);
                        }
                        $lastNodeValue = $value->nodeValue;
                        array_pop($tem);
                        array_push($word,$tem);
                        $tem = array();
                    }
    ...
    array_unshift($tem,$lastNodeValue);
    array_pop($tem);
    array_push($word,$tem);
//    var_dump($word);
}
